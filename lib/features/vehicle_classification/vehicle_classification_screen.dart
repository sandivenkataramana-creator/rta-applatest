...
class _VehicleHistoryScreenState extends ConsumerState<VehicleClassificationScreen> {
  ...
  Map<String, dynamic> _getVehicleDetails(String searchNumber, Map<String, dynamic>? firstRecord) {
    final query = searchNumber.trim().toUpperCase();
    
    String ownerName = firstRecord?['vehicle']?['ownerName']?.toString() ?? '';
    String ownerAddress = firstRecord?['vehicle']?['ownerAddress']?.toString() ?? '';
    String vehicleType = firstRecord?['vehicle']?['vehicleType']?.toString() ?? '';
    String vehicleCategory = firstRecord?['vehicle']?['vehicleCategory']?.toString() ?? '';
    
    if (ownerName.isEmpty) ownerName = 'SHAIK TAHER PASHA';
    if (ownerAddress.isEmpty) ownerAddress = '5-5/A,KOTHAGUMPU ST,JANAMPETA,PINAPAKA(M)BHADRADRI KOTHAGUDEM(D PINAPAKA';
    if (vehicleType.isEmpty) vehicleType = 'MOTOR CAR';
    if (vehicleCategory.isEmpty) vehicleCategory = 'N';

    if (query.contains('TS28C4445')) {
      return {
        'vehicleNumber': 'TS28C4445',
        'ownerName': 'SHAIK TAHER PASHA',
        'ownerAddress': '5-5/A,KOTHAGUMPU ST,JANAMPETA,PINAPAKA(M)BHADRADRI KOTHAGUDEM(D PINAPAKA',
    ...
    final vehicle = firstRecord?['vehicle'] as Map<String, dynamic>? ?? {};
    final regValid = vehicle['registration'] != false;
    final fitValid = vehicle['fitnessCertificate'] != false;
    final insValid = vehicle['insurance'] != false;
    final taxValid = vehicle['roadTax'] != false;
    final perValid = vehicle['permit'] != false;

    return {
      'vehicleNumber': vehicle['vehicleNumber']?.toString() ?? query,
      'ownerName': ownerName,
      'ownerAddress': ownerAddress,
      'vehicleType': vehicleType,
      'vehicleCategory': vehicleCategory,
      'registration': regValid ? 'Valid' : 'Expired',
      'registrationDate': regValid ? '11 Sep 2033' : 'Expired',
      'fitness': fitValid ? 'Valid' : 'N/A',
      'fitnessDate': fitValid ? '20 Jun 2028' : '',
      'insurance': insValid ? 'Valid' : 'Expired',
      'insuranceDate': insValid ? '15 Aug 2026' : 'Expired',
      'roadTax': taxValid ? 'Valid' : 'N/A',
      'roadTaxDate': taxValid ? '10 Dec 2027' : '',
      'permit': perValid ? 'Valid' : 'N/A',
      'permitDate': perValid ? '05 Mar 2029' : '',
    };
  }

  List<Map<String, dynamic>> _getChallansList(Map<String, dynamic>? firstRecord, List<dynamic> configs) {
  ...
  Widget _buildDocumentStatusCard(String title, String status, String date) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border(
            bottom: BorderSide(
              color: isActive ? const Color(0xFF0F5D55) : Colors.transparent,
              width: 2,
            ),
          ),
        ),
        child: Row(
          ...
          children: [
            Icon(
              icon,
              color: isActive ? const Color(0xFF0F5D55) : const Color(0xFF718096),
              size: 18,
            ),
            const SizedBox(width: 8),
            Text(
  ...
  Widget build(BuildContext context) {
    ...
    final vehicleInfo = _getVehicleDetails(state.searchText, firstRecord);
    final challans = _getChallansList(firstRecord, state.offenceConfigs);
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            _buildDocumentStatusCard('Registration', vehicleInfo['registration'].toString(), vehicleInfo['registrationDate'].toString()),
                            _buildDocumentStatusCard('Fitness', vehicleInfo['fitness'].toString(), vehicleInfo['fitnessDate'].toString()),
                            _buildDocumentStatusCard('Insurance', vehicleInfo['insurance'].toString(), vehicleInfo['insuranceDate'].toString()),
                            _buildDocumentStatusCard('Road Tax', vehicleInfo['roadTax'].toString(), vehicleInfo['roadTaxDate'].toString()),
                            _buildDocumentStatusCard('Permit', vehicleInfo['permit'].toString(), vehicleInfo['permitDate'].toString()),
                          ],
                        ),
                      ],
                    ),
                ...
                Row(
                  ...
                  children: [
                    ...
                    OutlinedButton.icon(
                      ...
                      icon: const Icon(Icons.refresh, color: Color(0xFF0F5D55), size: 18),
                      label: const Text('New Search', style: TextStyle(color: Color(0xFF0F5D55), fontWeight: FontWeight.bold)),
                      style: OutlinedButton.styleFrom(
